...
1. **Never add `requireApprovedUser()` (or any login redirect) to a tool page.** Tool pages use `getProfile()` from `js/auth.js` (null for visitors, never redirects) and call `logEvent('<tool>','view')` from `js/analytics.js`. The ONLY hard-gated pages are `/quote-log` (`requireApprovedUser('/login.html?reason=savelog')`) and `/game` (LEN-335, Steve 6/12 — the Deal Desk quiz is a deliberately gated easter egg so plays are attributable; it is NOT a tool page and its gate is NOT a regression).
...
5. **Save gate:** `saveEstimate()` (js/quote-log.js) dispatches `lp:estimate-saved`; `js/save-gate.js` handles the anonymous sign-in offer and the post-login re-save (`post-login-action`/`post-login-data` consumed on the TOOL page — never clear them in auth-callback; only `auth-return-to` is consumed there, BEFORE the redirect). New calculators must include `quote-log.js`, `save-gate.js`, `analytics.js`, and `completion-cue.js` (+ `LPCompletionCue.init` — spec in `CALCULATORS.md §18`).
6. **Analytics:** `usage_events` columns are `tool` / `event` / `session_id` / `metadata` / `user_id` (NOT tool_name/event_type). Tool keys are frozen (LEN-143). Anonymous inserts depend on the `anon_insert_usage_events` RLS policy (run 2026-06-11). Never put `params`/PII in metadata. `logEvent` must never throw. **Every user-facing page fires `logEvent('<slug>','view')` on open** (LEN-177 completed the sweep — calcs, waterfall, legislation, naics, plus `home`, `glossary`, `underwriting-guide`). Use `logEvent` from `js/analytics.js`, NOT bare `logUsage` from auth.js — `logUsage` returns early without a session, so it never records anonymous visitors.
